...
type
        ...
        procedure setAdicionaTalhao(IdKeyProp : Integer; IdKeyTalhao : Integer);
...
procedure TPropriedadeRural.setAdicionaTalhao(IdKeyProp : Integer; IdKeyTalhao : Integer);
var
        Qry1       : TFDQuery;
        Sql        : string;
begin
        try
             ...
             Qry1.Close;
             Sql := 'UPDATE Talhao set pr_idPropriedadeRural = '+IntToStr(IdKeyProp)+', tbl_status = ''DISTRIBUIDA'','+
             ' tbl_checkbox = 0 WHERE tbl_idTalhao = '+IntToStr(IdKeyTalhao)+'';
             Qry1.SQL.Clear;
             Qry1.SQL.Add(Sql);
             Qry1.ExecSQL;
             //Qry1.Active := True;
...
begin
        if fMsgPadrao(7,3,15,1,EmptyStr).RespBt then
        begin
